...
pub async fn build(settings: &WebServerSettings) -> Result<SqlPool> {
    ...
    let pool = Pool::builder()
        ...
        .max_lifetime(settings.services.postgres.pool.max_lifetime.map(|duration| duration.to_std().unwrap()))
        .idle_timeout(settings.services.postgres.pool.idle_timeout.map(|duration| duration.to_std().unwrap()))
        .connection_timeout(settings.services.postgres.pool.connection_timeout.to_std().unwrap())
        .reaper_rate(settings.services.postgres.pool.reaper_rate.to_std().unwrap())
        .build(manager)
        .await
