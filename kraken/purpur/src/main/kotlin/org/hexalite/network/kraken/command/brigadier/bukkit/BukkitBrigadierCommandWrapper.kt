...
inline fun CommandSender.toStack(): CommandSourceStack = VanillaCommandWrapper.getListener(this)

class BukkitBrigadierCommandWrapper(
    private val _plugin: KrakenPlugin, val kraken: KrakenCommand<CommandSourceStack>
    ...
    override fun execute(sender: CommandSender, commandLabel: String, args: Array<out String>): Boolean {
        ...
        try {
            ...
            if (ex.input != null && ex.cursor >= 0) {
                ...
                val error = Component.empty().withStyle(ChatFormatting.GRAY).withStyle {
                ...
                if (cursor < ex.input.length) {
                    error.append(
                        Component.literal(ex.input.substring(cursor)).withStyle(
                            ChatFormatting.RED, ChatFormatting.UNDERLINE
                        )
                    )
                }
                error.append(
                    (Component.translatable("command.context.here").withStyle(
